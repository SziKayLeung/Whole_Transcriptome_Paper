...
#SBATCH --array=0-5 # 6 samples
#SBATCH --output=All_Batch_Processing_OldChem-%A_%a.o
#SBATCH --error=All_Batch_Processing_OldChem-%A_%a.e
...
SAMPLES_NAMES=(FetalCTX2 FetalCTX4 FetalHIP1 FetalHIP2 FetalSTR1 FetalSTR2)
cd $FUNCTIONS
cat All_Human_RawData_OldChem.txt
# remove comments in raw.txt (https://kvz.io/blog/2007/07/11/cat-a-file-without-the-comments/)
BAM_FILES=(`cat "All_Human_RawData_OldChem.txt" | egrep -v "^\s*(#|$)"`)
...
run_CCS ${BAM_FILE} ${SAMPLE} $Isoseq3_WKD/CCS oldchem
