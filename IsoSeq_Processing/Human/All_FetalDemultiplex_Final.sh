...
#SBATCH --output=All_FetalDemultiplex_Final.o
#SBATCH --error=All_FetalDemultiplex_Final.e
...
# 26/06/2021: Fetal Samples merged
...
FETAL_SAMPLES_NAMES=(FetalCTX1 FetalCTX2 FetalCTX3 FetalCTX4 FetalCTX5 FetalHIP1 FetalHIP2 FetalSTR1 FetalSTR2)
...
merging_at_refine $Isoseq3_WKD/REFINE $Isoseq3_WKD/MERGED_CLUSTER AllFetal ${FETAL_SAMPLES_NAMES[@]}
...
run_map_cupcakecollapse AllFetal $Isoseq3_WKD/MERGED_CLUSTER $PostIsoseq3_WKD/MAP $PostIsoseq3_WKD/TOFU
...
demux $PostIsoseq3_WKD/TOFU/AllFetal.collapsed.read_stat.txt $FUNCTIONS/All_Human_Demultiplex.csv $PostIsoseq3_WKD/TOFU/AllFetal.Demultiplexed_Abundance.txt
...
run_sqanti2 AllFetal.collapsed AllFetal.collapsed.gff $PostIsoseq3_WKD/TOFU intropolis NA NA $PostIsoseq3_WKD/TOFU/AllFetal.Demultiplexed_Abundance.txt $PostIsoseq3_WKD/SQANTI2/GENOME
...
TAMA_remove_fragments $PostIsoseq3_WKD/SQANTI2/GENOME/AllFetal.collapsed_classification.filtered_lite.gtf AllFetal $PostIsoseq3_WKD/TAMA/GENOME
...
TAMA_sqanti_filter $PostIsoseq3_WKD/TAMA/GENOME/AllFetal.bed $PostIsoseq3_WKD/SQANTI2/GENOME AllFetal$sqname"_classification.txt" AllFetal$sqname".gtf" AllFetal$sqname".fasta" AllFetal$sqname"_junctions.txt" AllFetal $PostIsoseq3_WKD/SQANTI_TAMA_FILTER/GENOME
...
sed 's/transcript_id \([^;]\+\)/transcript_id \"\1\"/g' AllFetal_sqantitamafiltered.classification.gtf | sed 's/gene_id \([^;]\+\)/gene_id \"\1\"/g' | sed 's/gene_name \([^;]\+\)/gene_name \"\1\"/g' | sed 's/ref_gene_id \([^;]\+\)/ref_gene_id \"\1\"/g' > AllFetal_sqantitamafiltered.final.classification.gtf
